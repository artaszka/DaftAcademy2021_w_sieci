...
def get_pair_candles(
    ...
    all_candles = []
    for row in json:
        one_candle = Candle(open_time=row[0], close_time=row[6], open=float(row[1]), high=float(row[2]), low=float(row[3]), close=float(row[4]), volume=float(row[5]))
        all_candles.append(one_candle)

    print(all_candles)
    return all_candles


get_pair_candles("ETHUSDT", interval="1h", limit=2, start_time=datetime.datetime(2021, 12, 1), end_time=datetime.datetime(2021, 12, 6))



get_pair_candles("ETHUSDT", interval="1w", limit=3, start_time=datetime.datetime(2021, 12, 1),
